...
struct Arvore {
	...
	Arvore* criaFilho(T chave) {
		filhos[num_filhos++] = new Arvore<double, N >(chave, profundidade+1);
	...
	Arvore* geraMetade() {
		Arvore<double, N> * metade = new Arvore(chave/2, profundidade);
		for(int i = 0; i < num_filhos; ++i)
...
int main() {
    ...
    Arvore<double, 3> * R = new Arvore<double, 3>(5); //5 será a raiz
	...
	Arvore<double, 3> * M = R->geraMetade();
